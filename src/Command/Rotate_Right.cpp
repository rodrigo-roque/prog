    ...
    namespace command {
        ...
        Image *R_Right::apply(Image *img) {
            ...
            for (int y = 0; y < h; y++) { //Percorre os pixeis da imagem original coluna por coluna, linha por linha
                for (int x = 0; x < w; x++) {
                    //A linha da imagem nova será (new_h - 1 - x), pois este é a posição espelhada horizontalmente à posição vertical original
                    // e a coluna da imagem nova será "x"
                    rotated_right->at(new_w - 1 - y, x) = img->at(x, y); // Acede ao pixel na posição (x, y) da imagem rodada e altera para o valor da imagem inicial
                }
            }
