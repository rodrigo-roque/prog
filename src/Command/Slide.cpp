        ...
        Image *Slide::apply(Image *img) {
            ...
            for (int y = 0; y < h; y++) { //Percorre os pixeis da imagem linha por linha
                for (int x = 0; x < w; x++) { //Percorre os pixeis da imagem coluna por coluna
                    int new_x = (x + x_offset) % w; //Calcula a nova posição x após o deslocamento circular
                    //Ex: para w=5, x=4 e x_offset = 2 -> (4 + 2) % 5 = 6 % 5 = 1
                    if (new_x < 0) new_x += w; //Garante que o índice é positivo

                    int new_y = (y + y_offset) % h; //Calcula a nova posição y após o deslocamento circular
