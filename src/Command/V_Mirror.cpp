//
// Created by Tiago on 03/05/2025.
//

#include "Command/V_Mirror.hpp"
#include "Image.hpp"
#include "Color.hpp"
#include <sstream>

namespace prog {

    namespace command {

        VMirror::VMirror() : Command("v_mirror") {}

        VMirror::~VMirror() {}

        Image *VMirror::apply(Image *img) {
            int w = img->width(); // Guarda a largura da imagem
            int h = img->height(); // Guarda a altura da imagem
            for (int y = 0; y < h/2; y++) { //Percorre os pixeis da imagem linha por linha e inverte as cores de cada um
                for (int x = 0; x < w; x++) {
                    Color &up = img->at(x, y); // Acede ao pixel na posição (x, y)
                    Color &down = img->at(x, h-1-y); // Acede ao pixel na posição (x, height()-1-y) -> OPOSTO VERTICAL
                    Color temp = up; //Variável temporária para guardar "up"
                    up = down;
                    down = temp;
                }
            }
            return img;
        }
    }
}